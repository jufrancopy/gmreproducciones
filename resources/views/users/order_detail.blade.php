                            ...
                            <div class="detail_shipping_info">
                                @if ($order->o_type == 0)
                                    @if (!is_null(Auth::user()->getAddressDefault))
                                        <p><strong>Estado: </strong> {{ $order->getAddressDefault->getState->name }} </p>
                                        <p><strong>Ciudad: </strong> {{ $order->getAddressDefault->getCity->name }} </p>
                                        <p><strong>Barrio:
                                            </strong>{{ kvfj($order->getAddressDefault->addr_info, 'add1') }}
                                        </p>
                                        <p><strong>Dirección: </strong>,
                                            {{ kvfj($order->getAddressDefault->addr_info, 'add2') }},
                                            {{ kvfj($order->getAddressDefault->addr_info, 'add3') }}</p>
                                        <p><strong>Referencia:
                                            </strong>{{ kvfj($order->getAddressDefault->addr_info, 'add4') }}
                                        </p>
                                    @else
                                    @endif
                                @endif
